...
TABLE_DESCRIPTIONS = {
    ...
    "v_sentence_terms": "View over sentences: prison_months (TOTPRISN with the special codes >= 9990 removed), term_type (months / zero months / life / death / prison with no term stated / under one day / missing) and prison_imposed (PRISDUM, the codebook's imprisonment indicator)",
}
...
def run_validation(con, n_years: int) -> int:
    ...
    ck.query(con, "v_sentence_terms: prison_imposed populated (> 99%)", "SELECT COUNT(prison_imposed) * 1.0 / COUNT(*) FROM v_sentence_terms", lambda v: v > 0.99, lambda v: f"{v:.2%}")
    ck.query(con, "v_sentence_terms: every stated prison term has prison_imposed", "SELECT COUNT(*) FROM v_sentence_terms WHERE term_type = 'months' AND prison_imposed = FALSE", lambda n: n == 0, str)
...
def main():
    ...
    # TOTPRISN = 0 means "no prison OR less than one month ordered" (codebook), so it is
    # NOT a no-prison indicator: 229,535 zero-month rows have PRISDUM = 1. Imprisonment
    # incidence comes from PRISDUM, exposed here as prison_imposed.
    con.execute("""
        ...
        SELECT fiscal_year, USSCIDN,
               ...
               CASE WHEN TOTPRISN IS NULL THEN 'missing'
                    WHEN TOTPRISN = 0 THEN 'zero months (no prison or under one month)'
               ...
               CASE WHEN PRISDUM = 1 THEN TRUE WHEN PRISDUM = 0 THEN FALSE END AS prison_imposed,
