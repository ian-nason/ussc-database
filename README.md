...
con.sql("""
    SELECT fiscal_year, COUNT(*) AS defendants,
           ROUND(AVG(TOTPRISN), 1) AS avg_prison_months,   -- 0 for probation-only
           ROUND(100.0 * COUNT(SENTTOT) / COUNT(*), 1) AS pct_with_prison
...
- **`SENTTOT` is total prison months and is NULL for probation-only and zero-month
  sentences** (the Commission's convention), so `AVG(SENTTOT)` is the mean among prison
  sentences; use `TOTPRISN` (0 for probation) for all sentenced defendants. Life sentences are
  coded 470 in earlier years and 9996/9997 later; read the codebook before averaging.
